...
model = dict(
    ...
    data_preprocessor=dict(
        ...
        bgr_to_rgb=True),
    backbone=dict(
        type='HourglassNet',        # HourglassNet 사용
        downsample_times=5,         # 다운샘플링 5번
        num_stacks=2,
...
train_dataloader = dict(
    batch_size=6,           # 메모리 부족으로 batch size = 6
    num_workers=2,
...
optim_wrapper = dict(
    _delete_ = True,
    type='OptimWrapper',
    optimizer=dict(type='Adam', lr=1.25e-4))    # optimizer와 learning rate 지정
